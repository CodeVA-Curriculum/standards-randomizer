<script>
    ...
    export let results; // this is exposed to the App

    // figure out the state of the button
    let buttonState = "spinner";
    $: if(grades.length == 0 || subjects.length == 0) {
        buttonState = "spinner";
    } else if(gradeForm && subjectForm && checkPair(gradeForm, subjectForm)) {
        buttonState = "ready";
    ...
    let go = function (g, s) {
        if(buttonState == "ready") {
            ...
            axios.get('https://standards-api.herokuapp.com/api/v1.0/standards/get-pair', {
                params: {
                    grade: gradeForm.toUpperCase(),
                    subject: subjectForm.toUpperCase()
                }
            })
            .then(function (res) {
    ...
    onMount(async () => {
        ...
        .then(function (res) {
            ...
            grades = res["data"];
            if(subjects.length > 0) { buttonState = "prequery" }
        })
        ...
        .then(function (res) {
            ...
            // compile grade levels from response
            subjects = res["data"];
            if(grades.length > 0) { buttonState = "prequery" }
        })
        .catch(function (error) {
            console.log(error);
        })
